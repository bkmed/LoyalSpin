...
export default function AdminRoulette() {
  ...
  const [segments, setSegments] = useState<Partial<RouletteSegment>[]>(
    config?.segments || Array(8).fill({ label: 'Perdu', probability: 10, isGift: false })
  );
  
  const handleSave = () => {
    // Dispatch save action to update in firebase
    alert('Configuration de la roulette sauvegardée.');
  };
  ...
  return (
    <View className="max-w-5xl">
      ...
      <Text className="text-slate-500 mb-8">Définissez les 8 segments, les probabilités (total 100%) et les cadeaux.</Text>
      
      <View className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 mb-8">
        <Text className="text-xl font-bold dark:text-white mb-6">Segments de la roue</Text>
        
        {segments.map((seg, idx) => (
          <View key={idx} className="flex-row items-center space-x-4 mb-4 border-b border-slate-100 dark:border-slate-700 pb-4">
            ...
            <View className="flex-1">
              <TextInput
                ...
                placeholder="Texte (Ex: Cadeau Mystère)"
            ...
            <View className="w-24">
              <TextInput
                ...
                onChangeText={(text) => {
                  const newSegs = [...segments];
                  newSegs[idx] = { ...newSegs[idx], probability: parseInt(text) || 0 };
                  setSegments(newSegs);
                }}
            ...
            <View className="flex-row items-center w-24">
              <Text className="mr-2 text-slate-600 dark:text-slate-400">Cadeau</Text>
              <Switch
        ...
        <View className="flex-row justify-between items-center mt-6">
          <Text className="text-slate-500">Total Probabilité: {segments.reduce((acc, curr) => acc + (curr.probability || 0), 0)}% (doit être 100%)</Text>
          <TouchableOpacity onPress={handleSave} className="bg-[#1E3A5F] px-6 py-3 rounded-lg shadow-md">
            <Text className="text-white font-bold">Sauvegarder les segments</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
